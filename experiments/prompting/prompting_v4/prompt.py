...
if __name__ == "__main__":
    ...
    with open("output.txt", "w") as f:
        ...
        Provide your response only as a JSON object with keys "passed" with a boolean value indicating whether the candidate passed the exam for the given level and "level" with estimated level value of the CEFR level (A1, A2, B1, B2, C1). Bear in mind that the candidate can only pass the exam if the estimated level is equal or greater to the exam level. Only 5% of candidates fail the exam. Bear in ming that the transcripts are automatically generated and many errors are due to the automatic transcription and not the candidate's performance.
